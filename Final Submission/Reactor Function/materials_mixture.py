...
def mixture(u238, u235, boron, hw):    # input volume percentages for each material
    ...
    total_cs = np.array([sum(i) for i in zip(*prpl_cs)])    # sum each column for total cross-sections
    ...
    n_per = eff_den * (10**22)    # atoms/cm^3 for each material
    n = np.sum(n_per)    # total atoms/cm^3
    
    return prpl_cs, f_sf, f_sa, f_ss, s_sf, s_sa, s_ss, f_stot, s_stot, n_per, n
