...
def escape(neutron, count, reactorradius, f_sa, f_ss, s_sa, s_ss, f_stot, s_stot, prpl_cs, n, reflectivity = 0.5):
    ...
    for i in range(count):
        if neutron[2,0,i] == 1: # if the neutron is a fast neutron
            if rad[i] <= reactorradius:  # particle didn't leave
                ...
                if anothertempprob < reflectivity: # some portion of the time, the neutron is reflected back into the reactor
                ...
                else: # rest of the time the neutron escapes
                    ecount += 1
...
def baseline_escape(neutron, count, reactorradius, f_sa, f_ss, s_sa, s_ss, f_stot, s_stot, prpl_cs, n, reflectivity):
    ...
    fcount, acount, scount, ecount = \
        escape(neutron, count, reactorradius, f_sa, f_ss, s_sa, s_ss, f_stot, s_stot, prpl_cs, n, reflectivity)
