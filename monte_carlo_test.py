...
neutron = np.random.uniform(
    low=0, high=1.0, size=(2, 3, count)
)  # row 1: location; row 2: direction; multiple particles
neutron[1, :] = neutron[1, :] / (
    (np.sum((neutron[1, :]) ** 2)) ** 0.5
)  # direction vector has length 1
...
def escape(neut):
    ...
    neut[0, :] = np.add(
        neut[0, :], dist * neut[1, :]
    )  # change neutron position by dist in movement direction
